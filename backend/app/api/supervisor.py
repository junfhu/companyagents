...
async def pause_task(task_id: str, body: InterventionRequest, db: AsyncSession = Depends(get_db)):
    svc = SupervisorService(db)
    try:
        task = await svc.pause(task_id, body)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
...
async def resume_task(task_id: str, body: InterventionRequest, db: AsyncSession = Depends(get_db)):
    svc = SupervisorService(db)
    try:
        task = await svc.resume(task_id, body)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
    return {"ok": True, "data": {"task_id": task.id, "state": task.state.value}}
...
async def retry_task(task_id: str, body: InterventionRequest, db: AsyncSession = Depends(get_db)):
    svc = SupervisorService(db)
    try:
        task = await svc.retry(task_id, body)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
...
async def escalate_task(task_id: str, body: InterventionRequest, db: AsyncSession = Depends(get_db)):
    svc = SupervisorService(db)
    try:
        task = await svc.escalate(task_id, body)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
...
async def rollback_task(task_id: str, body: InterventionRequest, db: AsyncSession = Depends(get_db)):
    svc = SupervisorService(db)
    try:
        task = await svc.rollback(task_id, body)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
...
async def replan_task(task_id: str, body: InterventionRequest, db: AsyncSession = Depends(get_db)):
    svc = SupervisorService(db)
    try:
        task = await svc.replan(task_id, body)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
